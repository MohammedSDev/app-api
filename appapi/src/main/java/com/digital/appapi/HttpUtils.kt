...
inline fun <reified T : ResponseModel<*>> handleDataPacing(response: retrofit2.Response<ResponseBody>): T {
    ...
    if (response.isSuccessful) {
        ...
        if (result != null)
            ...
            throw RetrofitThrowable(error, error.message.toString())
        }
    } else {
...
//        val error =  converter<T>(response.errorBody())
    if (error != null)
        return RetrofitThrowable(error, error.message.toString())
    else {
...
//                statusCode = error?.statusCode
        ...
        return RetrofitThrowable(errorResponse, error?.message.toString())
    }
}
...
fun errorsHandling(throwable: Throwable): ErrorResponseModel {
    ...
    log(text = "errorsHandling, + ${throwable}")
    log(text = "errorsHandling,class simpleName + ${throwable.javaClass.simpleName}")
    log(text = "errorsHandling, message + ${throwable.message}")
